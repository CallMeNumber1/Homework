...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        char code[4];
        char log_file[3][100] = {"bash sysinfo/check_cpu.sh", "bash sysinfo/check_memory.sh", "bash sysinfo/disk.sh"};
        for (int i = 0; i < 3; i++) {
            // 将下面的while改成if就可实现多文件的传输
            if ((len = recv(confd, code, 3, 0)) > 0) {
                code[3] = '\0';
                if (strcmp(code, reqcode[i]) == 0) {
                    // 表示接受到了master对于cpu信息的请求
                    // 回复200表示存在cpu信息文件
                    // TODO:回复400表示不存在cpu信息文件, 进入另外的业务逻辑
                    // TODO:master每要求一种日志文件,是否都要新建一个监听端口?
                    printf("accept requst code %s\n", reqcode[i]);
                    send(confd, respcode[i], strlen(respcode[i]), 0);
                    // 这里是否需要新建一个监听状态的套接字
                    int data_sockfd = socket_create(6666 + i), data_confd;
                    ...
                    printf("begin send sysinfo %s to %s\n", reqcode[i], inet_ntoa(data_client_addr.sin_addr));
                    char buf[MAX_SIZE + 1] = {0};
                    FILE *p1 = popen(log_file[i], "r");
